...
export default function TiktokVideoDownloader(props) {
    ...
    const handleDownload = async() => {
        ...
        await axios.post('http://127.0.0.1:8000/api/download/tiktok-video/', {url: link})
        .then((res) => {
    ...
    return (
        <Container>
            <ToastContainer />
            <h2 className="m-5 text-center">TikTok Video Downloader</h2>
            <Container style={{display:'flex', justifyContent:'center'}}> 
                {!loading && <Form style={{width:'100%', display:'flex',justifyContent:'center'}}>
                    <Form.Group className="" controlId="link" style={{width:'50%'}}>
                        <Form.Control type="text" placeholder="Enter the link of the tiktok video" value={link} onChange={(e) => setData({...props.data, link: e.target.value})} />
                    </Form.Group>
                    <Form.Group>
            ...
            </Container>
            {
                ...
                <Container className="mt-5 mb-5">
                    ...
                    <Container style={{display:'flex', justifyContent:'center'}}>
                        <video width="60%" height="60%" controls style={{maxWidth:'600px', maxHeight:'300px'}}>
